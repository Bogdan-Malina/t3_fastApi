...
@app.get("/store/{phone}", response_model=list[schemas.Store])
def read_store_by_phone(phone, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    stores = crud.get_stores_by_phone(phone, db, skip=skip, limit=limit)
    return stores


@app.get("/order/{phone}", response_model=list[schemas.Order])
def read_order_by_phone(phone, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
